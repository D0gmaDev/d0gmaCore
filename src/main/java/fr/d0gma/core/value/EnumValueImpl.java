...
final class EnumValueImpl<T extends Enum<T>> extends ValueBase<T> implements EnumValue<T> {
    ...
    public void setIndexValue(int index) {
        if (index < 0 || index >= this.enumeration.length) {
            throw new IllegalStateException("ValueBase is incorrect for the given enum");
        }
